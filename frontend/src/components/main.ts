...
export class Main {
  ...
  private async getOperations(from: string, to: string): Promise<void> {
    if (this.getInfoRefreshToken) {
      const result: OperationsIncomeType[] | OperationsExpensesType[] =
      ...
      console.log(result);
      if (result) {
        const incomeTransactions: OperationsIncomeType[] = result.filter(
          (item) => item.type === "income"
        );
        const expenseTransactions: OperationsExpensesType[] = result.filter(
  ...
  private initChartIncome(income: OperationsIncomeType[]): void {
    ...
    const ctx: HTMLCanvasElement | string = document.getElementById(
      "myChart"
